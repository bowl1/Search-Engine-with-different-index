...
public List<Webpage> search(String query) {
    ...
    List<Webpage> result = QueryHandler.getMatchingWebsites(query.toLowerCase(), this);
    ...
    for (Webpage page : sortedResults) {
        ...
        for (String searchTerm : searchTerms) {
            ...
            Integer wordCount = page.getSearchTermCount().getOrDefault(lowerCaseSearchTerm, 0);

            // 获取文档长度
            int documentLength = page.getDocumentLength();

            // 获取文档总数
            int documentCount = sortedResults.size();
...
public int getDocumentCountContainingTerm(String searchTerm) {
    // 统一为小写进行匹配
    Set<Webpage> webpagesContainingTerm = searchDocuments(searchTerm.toLowerCase());
